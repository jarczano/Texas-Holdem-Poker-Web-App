            ...
            if not player.decision and player.live:
                ...
                if player.kind == 'human':
                    ...
                    if dict_options['raise']:
                        emit('player_option', [dict_options['check'], emit_call_value, min_raise, max_raise])
                    else:
                        emit('player_option', [dict_options['check'], emit_call_value, False, False])
                ...
                # update info
                emit('update_bet', [player.name, player.bet_auction])
                emit('update_stack', [player.name, player.stack])
                emit('update_pot', [sum([player.input_stack for player in player_list])])

                if player.name != "Alice":
                    if decision == 'raise':
                        emit('opponent_decision', ['Raise', chips])
                    else:
                        emit('opponent_decision', [decision, 0])
